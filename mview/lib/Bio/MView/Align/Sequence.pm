...
# $Id: Sequence.pm,v 1.23 2013/09/27 00:54:23 npb Exp $
...
sub color_by_type_in_match {
    ...
    my %mindex = ();
    map { $mindex{$_}++ } @{ $self->{string}->findall($par{find}) };

    ## obsolete code to find all exact string matches
    ##    my @mfind = split(//, uc $par{'find'});
    ##    my $mpos = 0; ##
    ##    my @mtmp = (); ##
    ##    my %mindex = (); ##
    ##
    ##    warn($self->{string}->string, "\n");
    ##    warn("FIND=$par{find}\n");
    ##
    ##    #test for match string
    ##    for ($end=$self->length+1, $i=1; $i<$end; $i++) {
    ##	      $c = uc $self->{'string'}->raw($i);
    ##        
    ##      MATCH:
    ##        while (1) {
    ##            if ($mfind[$mpos] eq $c) {
    ...
    ##                }
    ##                last MATCH;
    ##            } elsif ($self->{'string'}->is_space($c)) {
    ##                last MATCH;
    ##            } elsif ($self->{'string'}->is_non_sequence($c)) {
    ##                last MATCH;
    ##            } elsif ($mpos > 0) { #reset
    ##                @mtmp = ();
    ##                $mpos = 0;
