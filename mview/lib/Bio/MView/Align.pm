...
# $Id: Align.pm,v 1.46 2013/09/27 00:54:23 npb Exp $
...
sub load_colormaps {
	...
	die "load_colormaps(): color mapname undefined\n"
	    unless defined $map;
...
sub header {
    ...
    my $s = '';
    return $s    if $quiet;
    if ($self->{'coloring'} eq 'any' and $self->{'find'}) {
...
sub set_color_scheme {
    ...
    if ($self->{'coloring'} eq 'any' and $self->{'find'}) {
	$self->color_by_type_in_match('colormap'  => $self->{'colormap'},
                                      'colormap2' => $self->{'colormap2'},
                                      'symcolor'  => $self->{'symcolor'},
                                      'gapcolor'  => $self->{'gapcolor'},
                                      'css1'      => $self->{'css1'},
    ...
    if ($self->{'coloring'} eq 'find' and $self->{'find'}) {
	$self->color_by_type_in_match('colormap'  => $self->{'colormap'},
                                      'colormap2' => $self->{'colormap2'},
                                      'symcolor'  => $self->{'symcolor'},
                                      'gapcolor'  => $self->{'gapcolor'},
...
sub color_by_type_in_match {
	...
	next if exists $self->{'nopshash'}->{$self->{'index2row'}->[$i]->id};
	next if exists $self->{'hidehash'}->{$self->{'index2row'}->[$i]->id};
	$self->{'index2row'}->[$i]->color_by_type_in_match(@_);
    }
    $self;
}
