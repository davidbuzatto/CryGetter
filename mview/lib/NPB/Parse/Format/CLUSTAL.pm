...
# $Id: CLUSTAL.pm,v 1.18 2013/09/09 21:31:04 npb Exp $
...
my $CLUSTAL_ALIGNMENT      = '^\s*\S+\s+\S+(?:\s+\d+)?$';
...
sub new {
	...
	foreach $id (keys %{$self->{'seq'}}) {
	    $line = $self->{'seq'}->{$id};
	    if (length $line < $off) {
		$self->die("sequence '$id' is too short (expect $off)\n");
	    } elsif (length $line > $off) {
		$self->die("sequence '$id' is too long (expect $off)\n");
	    }
	}
    }
